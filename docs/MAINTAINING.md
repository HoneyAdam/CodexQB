...
- Goal compiler artifacts from `scripts/goal_run.py`, including deterministic spec IDs, unique invocation run IDs, project-specific active sub-plan and READY queue collectors, repo-contained output directories, explicit-output resume behavior, stage prerequisite blockers, render-time validation, unsafe glob/path overlap rejection, source snapshot digest integrity, and no silent overwrite;
- Apply-run artifacts from `scripts/apply_run.py`, including deterministic apply spec IDs, unique invocation run IDs, audit-derived task briefs, Step 4 readiness summaries, `no_action` mode, default `commit_policy: none`, unsafe command rejection, no-action queue rejection, task ID traversal rejection, transition/reconcile CLI event log enforcement, writer-lock consistency, external Superpowers readiness/reconcile validation, agent profile drift detection, VERIFIED evidence requirements, final review validation evidence, no silent progress overwrite, and spec-before-quality review order;
