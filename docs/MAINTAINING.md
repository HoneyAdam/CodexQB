...
- Goal compiler artifacts from `scripts/goal_run.py`, including deterministic spec IDs, unique invocation run IDs, project-specific active sub-plan and READY queue collectors, compiler version metadata, template bundle digests, implementation contract digests, repo-contained output directories, explicit-output resume behavior, bundled-validator stage prerequisite blockers, render-time validation, unsafe glob/path overlap rejection, source snapshot digest integrity, and no silent overwrite;
