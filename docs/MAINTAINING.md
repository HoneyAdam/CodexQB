...
When validating an extracted source package or copied tree that is not the exact Git root, `scripts/validate.sh` requires and verifies `PACKAGE-MANIFEST.json` before Git discovery and filesystem package hygiene. Use `make export-source-package` rather than an unmanifested directory copy. Set both `CODEXQB_VALIDATE_SKIP_UNITTESTS=1` and `CODEXQB_VALIDATE_SKIP_BEHAVIOR_SMOKE=1` only for this bounded package fallback; otherwise behavior smokes remain part of the normal gate.
