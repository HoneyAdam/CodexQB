...
CodexQB is explicit-invocation only: `policy.allow_implicit_invocation` is `false`, so ordinary Codex requests do not start this workflow. Invoke `$codexqb` explicitly or select CodexQB in the interface.
...
Goal prepare and render writes are restricted to one direct, non-symlink child of `Planner-docs/Goal-Runs/`; render may update only that run's `Goal-Prompt.md`. Traversal, a symlinked managed parent or run directory, and a symlink or special-file final target fail closed.
...
# The implementer changes only contract-authorized files and returns its structured result.
...
# Repeat run-validation for every planned ID, then call normalize-writer again with controller-bound receipt, change-set, contract, and patch digests.
...
# The read-only reviewer returns exactly one JSON payload and writes no Apply artifact.
...
python3 plugins/codexqb/skills/codexqb/scripts/apply_run.py record-agent --run-dir /path/to/project/.codexqb/apply-runs/<apply-run-id> --task-id <task-id> --role task_reviewer --review-phase spec --agent-id <reviewer-id> --status completed --actor controller --summary "spec review complete"
...
This is one ordered excerpt through the first spec receipt. Repeat `dispatch -> record spawned -> normalize-review -> record completed -> publish-review` for `quality`, required `security`, and `final`. The report payload contract is generic: `status: COMPLETE`, the matching `phase`, `task_id`, `reviewer_agent_id`, non-empty `evidence`, and `verdict`. Spec accepts `pass|fail|cannot_verify`; quality, security, and final accept `pass|fail|needs_fixes|cannot_verify`.

These commands are separate maintenance or fail-closed checks, not a sequential continuation of the excerpt:
...
The current controller-only protocol cannot satisfy the host-attestation prerequisite, so this `finalize` invocation is expected to fail closed.
...
Explicit Apply output directories must be direct, non-symlink children of `.codexqb/apply-runs/`. Destructive `--replace` additionally requires a structurally valid schema-v3 `Apply-Run.json`, a matching full-manifest marker, and a run/root-inode-bound registry receipt under `.codexqb/apply-runs/.codexqb-run-registry/`. That receipt is authenticated with a private HMAC key stored outside the repository at `~/.codex/codexqb-trust/apply-run-hmac-v1.key` by default. The adjacent `apply-run-hmac-v1.state.json` binds the initialized key ID; if the key is deleted or replaced, CodexQB reports recovery-required instead of silently rotating it and splitting existing receipts across trust domains. `CODEXQB_TRUST_ROOT` may point to an absolute, existing, owner-only directory for isolated tests or controlled CI; relative, symlinked, mounted, non-private, missing, or malformed trust stores fail closed. Run initialization stays on the freshly created run descriptor, writes the marker near the end, and publishes the signed receipt last. Copied, partial, self-attested, or synthetically registered repository artifacts are insufficient. Apply schema-v1 and schema-v2 artifacts are archive-only and cannot be validated, resumed, replaced, trusted-verified, or finalized; preserve/archive them and generate a new v3 run. Repository roots, unmanaged directories, nested descendants, symlinked paths, mounts, changed root/parent/run identities, incomplete manifests, and synthetic manifests are rejected before deletion. Managed run creation requires directory-descriptor and no-follow filesystem primitives; replacement additionally requires atomic no-replace rename support. Unsupported hosts fail closed instead of falling back to path-based deletion. A restore conflict leaves the random `.codexqb-delete-*` quarantine intact; a stale receipt, missing trust key, or interrupted deletion can leave a preserved but intentionally unregistered run. These recovery-required states block later replacement or recreation until the operator inspects and handles the preserved content manually. The HMAC protects against repository-contained, copied, and synthesized artifacts; a malicious process with access to the same OS account and trust key is outside this boundary.
...
`make check` also runs `evals/run_apply_behavior_smoke.py`, which drives `apply_run.py prepare`, `dispatch`, `record-agent`, `transition`, `capture-evidence`, `run-validation`, `normalize-review`, `publish-review`, `validate`, `recover-lock`, and `finalize` through subprocesses in a disposable repository. It proves the controller protocol and its fail-closed unattested boundary, not live Codex model identity or host-issued completion attestation. The downstream Goal/Apply dry run has the same limitation and must not be cited as a real attested reviewer-agent run.
