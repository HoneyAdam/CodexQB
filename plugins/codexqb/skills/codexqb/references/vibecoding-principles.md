...
CodexQB plans in a vibecoding-first style.
...
Long CodexQB runs should be phrased as Goal mode work: define the outcome, unchanged boundaries, validation checkpoints, stop gates, and the final summary expected from the agent.

A good Goal handoff is larger than one prompt but smaller than an unbounded backlog.
