...
These helpers write deterministic spec records with unique run directories inside the target repository and do not execute implementation, product validation, commit, push, PR, deploy, dependency install, or global Codex configuration changes. Apply runs record a `workspace_baseline` in `Apply-Run.json` with branch/base commit, Git status and diff hashes, untracked inventory hash, and non-Git file inventory hash when applicable.
