...
- **Deterministic Goal specs, unique Goal runs:** `scripts/goal_run.py` compiles source snapshots, active sub-plan inventory, structured Implementation Contracts, active sub-plan contract signals, Step 4 READY queues, subagent role plans, and canonical handoffs into `Goal-Run.json`, `Goal-Prompt.md`, and `Goal-Result.json` without executing commands. `goal_spec_id` is stable for the same source/mode/objective/scope, while `goal_run_id` is unique per invocation. Missing stage prerequisites or failing bundled validator gates produce a blocked result instead of an execution prompt.
