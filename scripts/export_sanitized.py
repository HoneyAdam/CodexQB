#!/usr/bin/env python3
"""Create a sanitized CodexQB source zip with package provenance.
...
def plugin_version(root: Path) -> str:
    plugin = root / "plugins/codexqb/.codex-plugin/plugin.json"
...
def release_blockers(root: Path, *, allow_dirty: bool, allow_head_mismatch: bool) -> list[str]:
    ...
    origin = origin_main_commit(root)
    head = git_commit(root)
    if origin and head != origin and not allow_head_mismatch:
...
def create_zip(
    ...
    *,
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr(
            f"CodexQB/{PACKAGE_MANIFEST_NAME}",
